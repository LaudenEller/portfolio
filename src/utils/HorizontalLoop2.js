...
// This code keeps the items centered in the carousel but bunches together the last and first items
  // Liekly culprit is the totalWidth calculations and ghost item formatting issues
    // TODO: Check widths, transform, and xValue for all carousel items
          // Check to see if StartX is messing with the ghost item at all

// It also makes the left side items invisible
  // Likely culprit is that the animation thinks the center of the page is the edge of the viewport and so it is clipping the items off
    // TODO: change overflow:hidden property
...
export function horizontalLoop(items, config) {
  ...
  const widths = items.map(item => parseFloat(gsap.getProperty(item, 'width', 'px')));
  let startX = (window.innerWidth - widths[0]) / 2;  // Center the first item on page load

  let curIndex = 0; // Declare curIndex to track the current carousel index
  let times = [],
      xPercents = [],
      pixelsPerSecond = (config.speed || 1) * 100,
      snap = config.snap === false ? (v) => v : gsap.utils.snap(config.snap || 1),
  ...
  // Calculate total width for the entire carousel
  totalWidth = items[length - 1].offsetLeft + (xPercents[length - 1] / 100) * widths[length - 1] - startX + items[length - 1].offsetWidth * gsap.getProperty(items[length - 1], "scaleX") + (parseFloat(config.paddingRight) || 0);

  // Loop over each item and set the correct position and opacity
  for (i = 0; i < length; i++) {
    item = items[i];
    curX = (xPercents[i] / 100) * widths[i];
    distanceToStart = item.offsetLeft + curX - startX;
    ...
    //TODO: check to see if these are the expected values (should they be negative?): distanceToLoop: -92.5 distanceToStart: -292.5
      // Values on second render = distanceToLoop: 567.5 distanceToStart: 367.5

      // WHy is the current index 3 instead of 1?
    tl.to(item, {
      xPercent: snap(((curX - distanceToLoop) / widths[i]) * 100),
      opacity: 0.3,  // Lower opacity when moving off-center
      duration: distanceToLoop / pixelsPerSecond,
    }, 0)
    .fromTo(item, {
      xPercent: snap(((curX - distanceToLoop + totalWidth) / widths[i]) * 100),
      ...
      xPercent: xPercents[i],
      opacity: 1, // Restore opacity when coming back into view
      duration: (curX - distanceToLoop + totalWidth - curX) / pixelsPerSecond,
      immediateRender: false,
      onUpdate: () => {
        // Calculate the distance from the center of the viewport
        const viewportCenter = window.innerWidth / 2;
        // Item position is coming back as NaN, why?
        const itemPosition = gsap.getProperty(item, 'x') + (widths[i] / 2); // center of the item
        const distanceFromCenter = Math.abs(viewportCenter - itemPosition); // absolute distance from the center

        // Adjust opacity based on the distance from the center of the viewport
        const opacityValue = Math.max(1 - (distanceFromCenter / viewportCenter), 0.3);
        gsap.to(item, { opacity: opacityValue, duration: 0.3 });
      },
    }, distanceToLoop / pixelsPerSecond)
    .add("label" + i, distanceToStart / pixelsPerSecond);
  ...
  // Function to move the timeline to a specific index
  function toIndex(index, vars) {
    vars = vars || {};
